...
function VolunteerRequests() {
  ...
  const fetchRequests = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE}/ngo/${ngoId}/volunteer-requests`);
      if (res.data.success) setRequests(res.data.requests);
      else setError("Failed to fetch volunteer requests.");
    } catch (err) {
  ...
  const handleStatusUpdate = async (requestId, status) => {
    try {
      const res = await axios.put(`${API_BASE}/volunteer/${requestId}/status`, { status });
      if (res.data.success) {
        setRequests((prev) =>
